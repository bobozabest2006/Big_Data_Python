...
def hbase_write(client, data, db_name, m_name=None, length=0, optional=False):
    if optional is True:
        fields = {
            "timestamp": list(data["Timestamp"]),
            "groupdate": [int(i + 100) for i in range(length)],
    ...
    start_time = time.time()
    to_hbase(df=df, con=client, table_name=db_name, key='df_key', cf=m_name)
    end_time = time.time()

    delta = end_time - start_time

    # for performance measuring only
    batch_write_time1.append(delta * 10**3)
    single_rec_write_time1.append(delta * 10**3 / length)
    pprint.pprint("HBase:Write Time {} records: {:.3f} ms".format(length, delta * 10**3))
    pprint.pprint("HBase:Write Time 1 record: {} ms".format(delta * 10**3 / length))
